...
class TestAYRUser:
    ...
    def test_when_superuser_return_empty_transferring_bodies(self, client):
        BodyFactory(Name="bar")

        groups = [
            "/ayr_user_type/view_all",
            "/transferring_body_user/foo",
            "/transferring_body_user/boo",
        ]
        user = AYRUser(groups)
        assert user.is_superuser
        assert len(user.transferring_bodies) == 0

    def test_when_superuser_return_single_transferring_bodies(self, client):
        body1 = BodyFactory(Name="foo")
        ...
        user = AYRUser(groups)
        assert user.is_superuser
        assert len(user.transferring_bodies) == 1
        assert user.transferring_bodies[0] == body1

    def test_when_superuser_return_multiple_transferring_bodies(self, client):
        body1 = BodyFactory(Name="foo")
        body2 = BodyFactory(Name="boo")
        groups = [
            "/ayr_user_type/view_all",
            "/transferring_body_user/foo",
            "/transferring_body_user/boo",
        ...
        user = AYRUser(groups)
        assert user.is_superuser
        assert len(user.transferring_bodies) == 2
        assert user.transferring_bodies[0] == body1
        assert user.transferring_bodies[1] == body2
