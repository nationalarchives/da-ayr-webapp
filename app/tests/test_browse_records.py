...
class TestBrowseRecords:
    ...
    def test_browse_records_js_sort_controls_have_label_associations(
        self, client: FlaskClient, mock_all_access_user, browse_files
    ):
        """
        Given browse records renders the JS sort controls
        When labels use the for attribute
        Then each label targets an existing control id
        """
        mock_all_access_user(client)
        ...
        js_sort_label = js_form.find("label", {"for": "sort-records-js"})
        js_sort_select = js_form.find("select", {"id": "sort-records-js"})
        js_per_page_label = js_form.find("label", {"for": "sort-js"})
        js_per_page_select = js_form.find("select", {"id": "sort-js"})

        assert js_sort_label is not None
        assert js_sort_select is not None
        assert js_per_page_label is not None
        assert js_per_page_select is not None

    def test_browse_records_noscript_sort_controls_have_label_associations(
        self, client: FlaskClient, mock_all_access_user, browse_files
    ):
        """
        Given browse records renders noscript sort controls
        When labels use the for attribute
        Then each label targets an existing control id
        """
        mock_all_access_user(client)
        ...
        noscript_sort_label = noscript_sort_form.find(
            "label", {"for": "sort-records-no-js"}
        )
        noscript_sort_select = noscript_sort_form.find(
            "select", {"id": "sort-records-no-js"}
        )
        noscript_per_page_label = noscript_per_page_form.find(
            "label", {"for": "sort-no-js"}
        )
        noscript_per_page_select = noscript_per_page_form.find(
            "select", {"id": "sort-no-js"}
        )

        assert noscript_sort_label is not None
        assert noscript_sort_select is not None
        assert noscript_per_page_label is not None
        assert noscript_per_page_select is not None
