...
function setupDOM(options = {}) {
  ...
  window.matchMedia = jest.fn().mockImplementation(query => {
    if (options.mediaQuery) return { matches: query.includes(options.mediaQuery) };
...
describe('tests for init.uv.js', () => {
  ...
  it('should initialize UniversalViewer with manifest', () => {
    require('./init.uv.js');
    document.dispatchEvent(new Event('DOMContentLoaded'));
    expect(window.UV.init).toHaveBeenCalledWith('uv', expect.objectContaining({ manifest: 'test-manifest' }));
  });
  ...
  ])('should set uv element styles for %s', (label, mediaQuery, expectedStyles) => {
    setupDOM({ mediaQuery });
    require('./init.uv.js');
      ...
      init: jest.fn(() => ({
        on: (event, handler) => {
          if (event === 'configure') {
            ...
            expect(config.modules.footerPanel.options.fullscreenEnabled).toBe(false);
            ...
            expect(cb).toHaveBeenCalledWith(expect.objectContaining({
              ...
              pdfHeaderPanel: expect.objectContaining({
                options: expect.objectContaining({ centerOptionsEnabled: false })
              })
            }));
          }
        },
