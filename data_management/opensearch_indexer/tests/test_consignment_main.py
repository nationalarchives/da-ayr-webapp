...
from opensearch_indexer.index_consignment.main import (
    BulkIndexMultipleConsignmentsError,
    consignment_indexer,
...
@mock_aws
def test_all_consignments_index_raises_error_after_collecting_failures(
    ...
    for c in range(2):
        ...
        sid = uuid4()
        bid = uuid4()
        cref = f"TDR-2024-ABCD-{c}"
        fid = uuid4()

        consignments.append((cid, cref))
        ...
        session.add_all(
            [
                File(
                    FileId=fid,
                    FileType="File",
                    FileName=f"file-{c}.txt",
                    FileReference=f"file-{c}",
                    FilePath=f"/tmp/file-{c}",
                    CiteableReference=f"cite-{c}",
                    ConsignmentId=cid,
                ),
                Consignment(
                    ConsignmentId=cid,
                    ConsignmentType="foo",
                    ConsignmentReference=cref,
                    SeriesId=sid,
                ),
                Series(SeriesId=sid, Name=f"series-{c}", BodyId=bid),
                Body(BodyId=bid, Name=f"body-{c}", Description="desc"),
            ]
        )
